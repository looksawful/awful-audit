...
if(!(Test-Path -LiteralPath $scriptsDir)){ throw "scripts directory not found: $scriptsDir" }
...
foreach($p in $profiles){ $dir=Split-Path -Parent $p; if($dir){ New-Item -ItemType Directory -Force -Path $dir | Out-Null }; if(!(Test-Path -LiteralPath $p)){ New-Item -ItemType File -Force -Path $p | Out-Null }; $txt=Get-Content -LiteralPath $p -Raw -ErrorAction SilentlyContinue; $marker='# awful-audit'; $block="$marker`nfunction au { & '$InstallDir\au.ps1' @args }`nfunction ау { & '$InstallDir\au.ps1' @args }"; if($txt -match [regex]::Escape($marker)){ $txt=[regex]::Replace($txt,"(?ms)^# awful-audit\r?\nfunction au \{.*?\}\r?\nfunction ау \{.*?\}\r?\n?",$block + "`n"); Set-Content -LiteralPath $p -Encoding utf8NoBOM -Value $txt } else { Add-Content -LiteralPath $p -Encoding utf8NoBOM -Value "`n$block`n" } }

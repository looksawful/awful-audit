...
function Show-AwfulHelp {
  ...
  Write-Host '  au all -Archive'
  Write-Host '  au all -Zip'
  Write-Host '  au all -ArchivePath _awful-audit\custom-audit.zip'
  Write-Host '  au full -Archive'
  Write-Host '  au css -Archive'
  Write-Host '  au all -Root A:\path\project -Archive'
  Write-Host ''
  Write-Host 'flags:'
  Write-Host '  -Output <path>         save report to a standalone text file'
  Write-Host '  -Archive, -Zip         create only a ZIP and copy it as a file'
  Write-Host '  -ArchivePath <path>    create the ZIP at a custom path'
